...
(function($) {
    ...
    /* === DIRETOR DA ANIMAÇÃO HERO FLASHIFY === */

    
window.addEventListener('load', () => {
    // Pega as referências dos elementos da animação
    ...
    if (!messyPile || !flashcardsDeck) {
        return;
    }

    // --- CENA DE INTRODUÇÃO ---
    // 1: A "bagunça" de ícones cai na tela
    setTimeout(() => {
        messyPile.classList.add('animate');
    }, 500);

    // 2: A transformação!
    setTimeout(() => {
        messyPile.classList.add('transform'); 
        flashcardsDeck.classList.add('animate'); 
    }, 2500); // Tempo para a transformação acontecer

    // 3: Limpeza e início do ciclo
    setTimeout(() => {
        messyPile.style.display = 'none';
        startFlashcardCycle();
    }, 3300);


    // --- CICLO CONTÍNUO DOS FLASHCARDS ---
    function startFlashcardCycle() {
        ...
        setInterval(() => {
            ...
            // 1. Vira o card para mostrar a resposta
            topCard.classList.add('is-flipped');

            // 2. Após um tempo, inicia a animação de saída
            setTimeout(() => {
                topCard.classList.add('sending-to-back');

                // 3. Ao final da animação, move o card para o fundo
                setTimeout(() => {
                    topCard.classList.remove('is-flipped');
                    topCard.classList.remove('sending-to-back');
